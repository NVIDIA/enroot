...
docker::daemon::import() (
    ...
    # Create a temporary directory and chdir to it.
    trap 'common::rmall "${tmpdir}" 2> /dev/null; docker rm -f -v "${container}" > /dev/null 2>&1' EXIT
    tmpdir=$(common::mktmpdir enroot)
    common::chdir "${tmpdir}"

    # Download the image (if necessary) and create a container for extraction.
    common::log INFO "Fetching image" NL
    container=$(docker create --platform "${arch}" "${image}")
    common::log

    # Extract and configure the rootfs.
    common::log INFO "Extracting image content..."
    mkdir rootfs
    docker export "${container}" | tar -C rootfs --warning=no-timestamp --exclude='dev/*' --exclude='.dockerenv' -px
